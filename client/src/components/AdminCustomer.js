...
const PostCustomer = ()=>{
    ...
    const DeleteCustomer = async(customer_id)=>{
      try {
        const query =  fetch(`http://localhost:5020/customer/${customer_id}`,{
          method : 'DELETE'
        });
        console.log(query);
      } catch (error) {
    ...
    const PostCustomer = async()=> {
      try {
        ...
        const query = fetch('http://localhost:5020/customer',{
          method : 'POST',
    ...
    const GetCustomers = async()=> {
      try {
        const get_cust = await fetch('http://localhost:5020/customer');
