...
const PostEmployee = ()=>{
  ...
  const DeleteEmp= async (username) => {
    try {
        const delete_emp = await fetch(`http://localhost:5020/employee/${username}`,{
            method : "DELETE"
        });
  ...
  const AddEmployee = async()=> {
    try {
      ...
      const query = fetch('http://localhost:5020/employee',{
        method : 'POST',
  ...
  const GetEmployees = async()=> {
    try {
      const query = await fetch('http://localhost:5020/employee');
