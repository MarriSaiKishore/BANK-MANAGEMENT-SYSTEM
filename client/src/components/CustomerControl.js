...
const CustomerControl = ()=>{
    ...
    const DeleteAccount = async(account_id)=>{
      try {
        const query = await fetch(`http://localhost:5020/accounts/${account_id}`,{
          method : 'DELETE'
        });
        console.log(query);
      } catch (error) {
    ...
    const AddAccount = async()=>{
      ...
      const query = await fetch('http://localhost:5020/accounts',{
        method : 'POST',
    ...
    const GetAccountDetails = async()=>{
      try {
        const query = await fetch(`http://localhost:5020/accounts/${id}`);
    ...
    const GetTransactions = async()=>{
      try {
        const customer_id = document.getElementById('customer_id_value').value;
        const query = await fetch(`http://localhost:5020/transaction/${customer_id}`);
        const data = await query.json();
        SetTransaction(data);
        console.log(data);
      } catch (error) {
    ...
    const GetCustomer = async()=>{
        try {
            ...
            const query = await fetch(`http://localhost:5020/customer/${username}`);
            const data = await query.json();
            console.log(data);
